...
const FROM_ADDRESS =
  process.env.RESEND_FROM_EMAIL || 'TREVOROS LABS <admissions@trevoros.com>';

async function dispatch(to: string, subject: string, text: string, html: string) {
...
function getCertPreviewUrl(performance: string): string {
  const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
  if (performance === 'EXCEPTIONAL') return frontendUrl + '/Certificate_Exceptional.png';
  if (performance === 'STRONG') return frontendUrl + '/Certificate_strong.png';
...
function btn(label: string, href: string, color: string = '#0ea5e9'): string {
...
export async function sendApplicationSubmittedEmail(to: string, name: string, track: string) {
...
export async function sendApplicationAcceptedEmail(to: string, name: string, track: string) {
  const trackLabel = track === 'BUILDER' ? 'Builder' : 'Foundation';
  const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
...
export async function sendApplicationRejectedEmail(to: string, name: string, track: string, adminNotes?: string) {
  const trackLabel = track === 'BUILDER' ? 'Builder' : 'Foundation';
...
export async function sendEnrollmentBatchEmail(
  to: string,
  name: string,
  track: string,
  batch: {
  ...
  const html =
    '<h2 style="color: #0f172a; margin-top: 0;">Enrollment Confirmed! 🎉</h2>'
    + '<p>Hello <strong>' + name + '</strong>,</p>'
